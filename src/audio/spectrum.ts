...
export function createFftJsSpectrumAnalyzer(FFT: FftJsConstructor): SpectrumAnalyzer {
  return (samples, sampleRateHz) => {
    ...
    const windowed = applyWindow(samples, hannWindow(fftSize));
    engine.realTransform(output, windowed);
    ...
    for (let bin = 0; bin < binCount; bin += 1) {
      ...
      const magnitude = (edgeScale * Math.hypot(real, imaginary)) / (fftSize * coherentGain);
